...
├── examples                         # examples to deploy contract and call its function on testnet
...
1. Install and import / require [`scryptjs` libary](https://github.com/scrypt-sv/scryptjs), which is a javascript SDK for integrating sCrypt smart contract.
...
Note that the `demo_desc.json` is a description file of the compiled contract, which could be generated by using the IDE or running this command in the project folder:

```
npm run compile-single -- contracts/demo.scrypt
```
...
1. Run an example file on testnet by commands like `node examples/demo.js`, and the output would appear in the console:
