...
const { bsv, buildContractClass, signTx, toHex, getPreimage, num2bin, PubKey, Bytes, Sig } = require('scryptjs');
const { inputIndex, inputSatoshis, tx, loadDesc, DataLen, dummyTxId } = require('../../helper');
...
describe('Test sCrypt contract Non-Fungible Token In Javascript', () => {
  ...
  before(() => {
    const Token = buildContractClass(loadDesc('nonFungibleToken_desc.json'))
    token = new Token()
